...
describe('Universe Contract', async (accounts) => {
  ...
  const signTeamForclosure = async (signer, to, tokenId, amount, unixTimestamp) => {

    let payload = AbiCoder.defaultAbiCoder().encode([ "uint", "uint", "uint", "address", "uint" ], [ tokenId, amount, unixTimestamp, to.address, 1 ])
    let payloadHash = ethers.keccak256(payload)
  ...
  it("should fail to mint team with wrong signature", async () => {
  ...
  it("should fail to mint team with wrong amount of ETH", async () => {
  ...
  it("should fail to mint team with wrong mint address", async () => {
  ...
  it("should fail to mint team with expired timestamp", async () => {
  ...
  it("should fail to mint team with wrong wallet", async () => {
  ...
  const mintTeam = async (contract, minterSignerAccount, to, tokenId, ethCost ) => {
  ...
  it("should fail to mint team if already minted", async () => {
  ...
  it("should burn diamonds as deposit", async () => {
    ...
    //Then burn them
    let tx2 = await diamondsContract.connect(accounts[0]).deposit(accounts[0].address, amount, tokenId)
  ...
  it("should fail to burn diamonds as universe if not minter", async () => {

    const { contract, diamondsContract, accounts } = await loadFixture(deployFunction)

    let amount = ethers.parseUnits('2', 'ether')

    await expect(diamondsContract.connect(accounts[4]).universeBurn(accounts[4].address, amount)).to.be.revertedWithCustomError(
      contract,
  ...
  for (let l of logs) {
    if (l.fragment.name === 'DepositToTeam') {
      try {
        ...
        assert.strictEqual(l.args[1].toString(), teamId.toString())
        assert.strictEqual(l.args[2].toString(), amount.toString())
        eventCount += 1
      } catch (ex) { }
    }
